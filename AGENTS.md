...
- **Project:** Customer Segmentation and Interpretation
...
- Training requires the external course CSV and runs with `customer-segmentation train --data data/raw/CA2-Customer-Data.csv`.

## Managed artifacts and data

- Treat `notebooks/customer_segmentation_analysis.ipynb` as the full source notebook and regenerate `notebooks/sections/` with `python scripts/split_notebook.py`.
- Keep raw/processed data, trained models, figures, metrics, and labelled outputs out of Git unless licensing, privacy, and assessment requirements explicitly allow them.

## Integration and claim boundaries

- The raw customer CSV is external to the repository.
- Clusters and interpretations remain dataset-specific and do not establish business lift.
- **Verified local capability:** Python 3.11/3.12, compile, lint, and security checks.
- **Known boundary:** Results remain dataset-specific.
